...
class LocalClassifier(Classifier):
    ...
    def fit(self, graph, train_indices):
        ...
        for node in training_nodes:
            features.append(np.array(node.feature_vector, dtype=float))
    ...
    def predict(self, graph, test_indices, conditional_node_to_label_map = None):
        ...
        '''
        labels = []
        nodes = [graph.node_list[i] for i in test_indices]

        for node in nodes:
            labels += self.clf.predict(np.array(node.feature_vector, dtype=float))
        return labels
